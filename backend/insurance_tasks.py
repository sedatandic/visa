...
def _charge_buckets(charged: list) -> dict:
    """Cekim yapilmis gorevleri ay anahtarina gore toplar."""
    buckets: dict = {}
    for task in charged:
...
def _charge_detail(task: dict) -> dict:
    """Gider tablosunda gosterilen tek cekim satiri."""
    steps = task.get("provider_steps") or {}
    return {
        "task_id": task["id"],
        ...
        "policy_no": steps.get("policy_no") or (steps.get("policy") or {}).get("police_no"),
...
async def expense_report(months: int = 12) -> dict:
    ...
    buckets = _charge_buckets(charged)
    rows = [
        {
            "month": key,
            "label": f"{key[5:]}.{key[2:4]}",
    ...
    return {
        "currency": "TRY",
        "items": rows,
        "recent": [_charge_detail(task) for task in charged[:25]],
        "totals": {
            "charged_try": round(sum(row["charged_try"] for row in rows), 2),
